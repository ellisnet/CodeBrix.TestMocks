...
using CodeBrix.TestMocks.DynamicProxy.Generators;
using CodeBrix.TestMocks.DynamicProxy.Internal;
...
namespace CodeBrix.TestMocks.DynamicProxy.Contributors; //was previously: namespace Castle.DynamicProxy.Contributors;
...
internal abstract class MembersCollector
{
    ...
    protected bool AcceptMethodPreScreen(MethodInfo method, bool onlyVirtuals, IProxyGenerationHook hook)
    {
        ...
        // Methods with by-ref-like types (e.g. Span<T>, ReadOnlySpan<T>) in their signature
        // cannot be intercepted, because by-ref-like types cannot be boxed to object for the
        // invocation arguments array. Skip these so the base implementation is used directly.
        if (HasByRefLikeType(method))
        {
            return false;
        }
    ...
    private static bool HasByRefLikeType(MethodInfo method)
    {
