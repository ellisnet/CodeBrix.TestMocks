...
static class Extensions
{
        ...
        where TOtherTypes : IReadOnlyList<Type>
    {
        ...
        // Also check the reverse direction: the method's parameter types may be
        // assignable from the callback's parameter types. This allows callbacks that
        // use more specific parameter types than the method signature, which is safe
        // when the setup's matchers constrain the actual argument types.
        if (otherTypes.CompareTo(method.GetParameterTypes(), exact: false, considerTypeMatchers: false))
        {
            return true;
        }

        if (invokeMethod != null && otherTypes.CompareTo(invokeMethod.GetParameterTypes(), exact: false, considerTypeMatchers: false))
        {
            return true;
        }
