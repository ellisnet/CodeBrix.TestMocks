...
public abstract partial class Mock : IFluentInterface
{
    ...
    static void VerifyNoOtherCalls(Mock mock, HashSet<Mock> verifiedMocks)
    {
        ...
        if (unverifiedInvocations.Any())
        {
            ...
            if (innerMocks.Any())
            {
                var allInvocations = mock.MutableInvocations.ToArray();
                ...
                for (int i = 0, n = unverifiedInvocations.Length; i < n; ++i)
                {
                    ...
                    // one call between this invocation and the next invocation of the same method.
                    // This uses globally unique sequence numbers to determine temporal ordering
                    // across mocks.
                    if (mock.MutableSetups.FindLastInnerMock(setup => setup.Matches(unverifiedInvocations[i])) is Mock innerMock)
                    {
                        var t0 = unverifiedInvocations[i].SequenceNumber;

                        // Find the sequence number of the next invocation of the same method:
                        long t1 = long.MaxValue;
                        foreach (var inv in allInvocations)
                        {
                            if (inv.SequenceNumber > t0 && inv.Method == unverifiedInvocations[i].Method)
                            {
                                t1 = inv.SequenceNumber;
                                break;
                            }
                        }

                        // An invocation is "transitive" if the inner mock received at least one
                        // invocation between this invocation and the next same-method invocation
                        // (or after this invocation if it is the last one of its kind):
                        var wasTransitiveInvocation = innerMock.MutableInvocations.ToArray()
                            .Any(inv => inv.SequenceNumber > t0 && inv.SequenceNumber < t1);
        ...
        List<Pair<Invocation, MethodExpectation>> invocationsToBeMarkedAsVerified)
    {
        ...
        foreach (var matchingInvocation in mock.MutableInvocations.ToArray().Where(part.IsMatch))
        {
            ...
            else
            {
                ...
                // Intermediate invocations are not marked as verified here; instead,
                // VerifyNoOtherCalls uses sequence-number-based temporal analysis to
                // determine which intermediate invocations were "transitive" (used to
                // reach an inner mock member) and which were standalone.
