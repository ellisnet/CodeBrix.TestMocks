...
class ConstantMatcher : IMatcher
{
    ...
    {
        if (object.ReferenceEquals(argument, constantValue))
        {
            return true;
        }

        // When both sides are mock objects, avoid calling Equals to prevent
        // infinite recursion if Equals has been set up on either mock.
        // Mock proxies use reference equality by default, so ReferenceEquals
        // above is sufficient for this case.
        if (argument is IMocked && constantValue is IMocked)
        {
            return false;
        }
