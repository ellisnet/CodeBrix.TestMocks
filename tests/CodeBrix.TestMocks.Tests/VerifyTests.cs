...
using CodeBrix.TestMocks.Tests.Helpers;
    ...
    public class VerifyTests
    {
        ...
        public void IncludesActualCallsInFailureMessage()
        {
            ...
            Assert.True(mex.Message.ContainsConsecutiveLines(
                "      VerifyTests.IFoo.Execute(\"ping\")",
                "      VerifyTests.IFoo.Echo(42)",
                "      VerifyTests.IFoo.Submit()",
                "      VerifyTests.IFoo.Save([1, 2, \"hello\"])"));
        }
        ...
        public void NullArrayValuesForActualInvocationArePrintedAsNullInMockExeptionMessage()
        {
            ...
            Assert.True(mex.Message.ContainsConsecutiveLines(
                @"      VerifyTests.IArrays.Method([""1"", null, ""3""])"));
        }
        ...
        public void LargeEnumerablesInActualInvocationAreNotCutOffFor10Elements()
        {
            ...
            var mex = Assert.Throws<MockException>(() => mock.Verify(_ => _.Method(null)));
            Assert.True(mex.Message.ContainsConsecutiveLines(
                @"      VerifyTests.IArrays.Method([""1"", null, ""3"", ""4"", ""5"", ""6"", ""7"", ""8"", ""9"", ""10""])"));
        }
        ...
        public void Enabling_diagnostic_file_info_leads_to_that_information_in_verification_error_messages()
        {
            ...
            Assert.Contains("VerifyTests.cs: line ", ex.Message);
        }
        ...
        public void Disabling_diagnostic_file_info_leads_to_that_information_missing_in_verification_error_messages()
        {
            ...
            Assert.DoesNotContain("VerifyTests.cs: line ", ex.Message);
        }
...
#pragma warning restore CS0618
        ...
        public void VerifyNoOtherCalls_can_tell_apart_transitive_and_nontransitive_usages_of_automocked_properties()
        {
            ...
            // The non-transitive access to `Bar` was not explicitly verified, so this should fail:
            Assert.Throws<MockException>(() => mock.VerifyNoOtherCalls());
        }

        [Fact]
        public void Verify_with_multi_dot_expression_marks_only_final_invocation_as_verified()
        {
            var mock = new Mock<IFoo>() { DefaultValue = DefaultValue.Mock };
            _ = mock.Object.Bar.Value;

            var outerInvocations = mock.MutableInvocations.ToArray();
            Assert.Single(outerInvocations);
            Assert.False(outerInvocations[0].IsVerified);

            mock.Verify(m => m.Bar.Value);

            // The intermediate `Bar` access on the outer mock should NOT be marked as verified
            // by Verify; transitive detection in VerifyNoOtherCalls handles it instead:
            Assert.False(outerInvocations[0].IsVerified);

            // The final `Value` access on the inner mock should be marked as verified:
            var innerMock = Mock.Get(mock.Object.Bar);
            var innerInvocations = innerMock.MutableInvocations.ToArray();
            Assert.Contains(innerInvocations, i => i.IsVerified);
        }

        [Fact]
        public void VerifyNoOtherCalls_succeeds_when_multiple_transitive_accesses_are_made()
        {
            var mock = new Mock<IFoo>() { DefaultValue = DefaultValue.Mock };
            mock.Object.Bar.Poke();
            mock.Object.Bar.Poke();

            mock.Verify(m => m.Bar.Poke(), Times.Exactly(2));
            mock.VerifyNoOtherCalls();
        }

        [Fact]
        public void VerifyNoOtherCalls_fails_when_nontransitive_access_follows_transitive_access()
        {
            var mock = new Mock<IFoo>() { DefaultValue = DefaultValue.Mock };
            mock.Object.Bar.Poke();
            _ = mock.Object.Bar;
            ...
            // The second `Bar` access was non-transitive (no inner mock member accessed after it),
            // so it should remain unverified:
            Assert.Throws<MockException>(() => mock.VerifyNoOtherCalls());
        }

        [Fact]
        public void Invocation_sequence_numbers_increase_across_different_mocks()
        {
            ...
            var invocations1 = mock1.MutableInvocations.ToArray();
            var invocations2 = mock2.MutableInvocations.ToArray();

            // Sequence numbers should be globally increasing regardless of which mock was invoked:
            Assert.True(invocations1[0].SequenceNumber < invocations2[0].SequenceNumber);
            Assert.True(invocations2[0].SequenceNumber < invocations1[1].SequenceNumber);
        }
...
namespace SomeNamespace
{
    ...
    public class VerifyExceptionsFixture
    {
        ...
        {
            var mock = new Mock<CodeBrix.TestMocks.Tests.VerifyTests.IFoo>();
